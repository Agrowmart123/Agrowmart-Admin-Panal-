...
import websiteContentService from "../../../api/websiteContentService"; // ← adjust path as per your folder structure
...
export default function BannersManagement() {
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      try {
        setLoading(true);
        const data = await websiteContentService.getAllByType("BANNER");
        setBanners(data || []);
      } catch (error) {
    ...
    const interval = setInterval(() => {
      setBanners((prev) =>
        prev.map((b) => {
          ...
          if (start && now < start) updatedStatus = "Scheduled";
          else if (start && end && now >= start && now <= end)
            updatedStatus = "Live";
          else if (end && now > end) updatedStatus = "Expired";

          return updatedStatus !== b.status
  ...
  const confirmDelete = async () => {
    try {
      ...
      setBanners((prev) => prev.filter((b) => b.id !== itemToDelete));
      toast.error("Banner deleted successfully!");
    } catch (error) {
  ...
  const handleSave = async (formData) => {
    try {
      ...
      let computedStatus = formData.status || "Draft";

      if (formData.status === "Published" || !formData.status) {
        if (start && now < start) computedStatus = "Scheduled";
        else if (start && end && now >= start && now <= end)
          computedStatus = "Live";
        else if (end && now > end) computedStatus = "Expired";
      }

      // Prepare payload matching backend fields
      const payload = {
        title: formData.title,
        subtitle: formData.description || "", // frontend description → backend subtitle
        type: "BANNER",
        status: computedStatus,
        imageUrl: formData.image || "",
        videoUrl: "", // not used in banners usually
        targetPlatform: formData.target || "Vendor App",
        targetAudience: formData.targeting || "All Vendors",
      ...
      let savedItem;
      if (editItem?.id) {
        savedItem = await websiteContentService.saveContent(
          payload,
          editItem.id,
        );
        setBanners((prev) =>
          prev.map((b) => (b.id === savedItem.id ? savedItem : b)),
        );
        toast.success("Banner updated successfully!");
      } else {
        savedItem = await websiteContentService.saveContent(payload);
  ...
  const filteredBanners = banners.filter((b) => {
    ...
    const matchesPlatform =
      platformFilter === "All" || b.targetPlatform === platformFilter;
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-4">
      <div className="bg-white border border-gray-200 rounded shadow-sm p-4 md:p-6">
        ...
        {/* Search & Filters */}
        <div className="mb-6">
          <div className="bg-gray-50 p-4 rounded-xl flex flex-col sm:flex-row sm:flex-wrap gap-4">
            <div className="w-full sm:flex-1">
              <input
                type="text"
                placeholder="Search banner campaigns..."
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="w-full px-4 py-2 rounded-lg bg-white text-black placeholder-gray-400 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-green-400"
              />
            </div>
            ...
            <div className="w-full sm:w-auto">
              <select
                value={platformFilter}
                onChange={(e) => setPlatformFilter(e.target.value)}
                className="w-full sm:w-48 px-4 py-2 rounded-lg bg-white text-black border border-gray-300 focus:outline-none"
              >
                <option value="All">All Platforms</option>
                <option value="Vendor App">Vendor App</option>
                <option value="User App">User App</option>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
      {items.map((b) => {
        const remaining = getRemainingTime(b.endDate);
        const ctr =
          b.views && b.clicks ? ((b.clicks / b.views) * 100).toFixed(1) : "0.0";
            ...
            <div className="p-4 space-y-4">
              <div>
                ...
                <div className="flex items-center gap-6 text-sm text-white/80 pt-2 border-t border-white/20">
                  ...
                  <div className="flex items-center gap-2">
                    <MousePointerClick size={16} />
                    <span>{(b.clicks / 1000).toFixed(1) || 0}k</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Percent size={16} />
                    <span>{ctr}% CTR</span>
...
function ContentModal({ item, onClose, onSave }) {
  ...
  useEffect(() => {
    if (item) {
      setForm({
        target: item.targetPlatform || "Vendor App",
        type: item.type || "Policy Update", // optional
        title: item.title || "",
        description: item.subtitle || "", // ← backend subtitle → frontend description
        targeting: item.targetAudience || "All Vendors",
        priority: item.priority || "High",
