...
// const quickActions = [
//   { label: "All Products", icon: Package, color: "bg-blue-500" },
...
//               <Bar dataKey="orders" fill="#3b82f6"  barSize={isMobile ? 14 : 40}/>
//             </BarChart>
//           </ResponsiveContainer>
//         </div>
//       </div>
...
//             <Bar dataKey="total" radius={[6, 6, 0, 0]} name="Total Sales"   barSize={isMobile ? 16 : 48}>
...
//   // Fetch product sales when filter changes
//   useEffect(() => {
//     if (!loading) {
//       fetchProductSales(filter);
//     }
//   }, [filter]);
...
//       if (result.success) {
//         const { stats, sellers, customers, products } = result.data;

//         // Set stats cards
//         setStatsCards(stats.stats || []);
//         setRegistrationData(stats.registrations || []);
//         setOrdersData(stats.orders || []);
        
//         // Set initial product sales (year view)
//         const yearSales = stats.productSales?.year || [];
//         setProductSalesData(
//           yearSales.map((item, index) => ({
//             ...item,
//             total: item.productA + item.productB,
//             fillColor: index % 2 === 0 ? "#5dbe2dff" : "#3b82f6",
//           }))
...
//   const fetchProductSales = async (filterType) => {
//     try {
//       const result = await dashboardApi.getProductSales(filterType);

//       if (result.success) {
//         const data = result.data.map((item, index) => ({
//           ...item,
//           total: item.total || (item.productA + item.productB),
//           fillColor: index % 2 === 0 ? "#5dbe2dff" : "#3b82f6",
//         }));
...
//   const handleQuickAction = (label) => {
//     if (label === "All Products") {
//       navigate("/admin/products", { state: { fromDashboard: true } });
//     } else if (label === "Send Notification") {
//       navigate("/notifications", { state: { fromDashboard: true } });
//     } else if (label === "Market Rates") {
//       navigate("/market-rates", { state: { fromDashboard: true } });
//     } else if (label === "Create Banner") {
//       navigate("/websites", { state: { fromDashboard: true } });
//     }
//   };
...
//           </div>
//           <div className="space-y-4">
//             {sellers.map((merchant, i) => (
//               <div key={merchant.id} className="flex items-start justify-between pb-1">
//                 <div className="flex gap-3">
//                   <div
//                     className={`${merchant.color} w-10 h-10 rounded-full flex items-center justify-center text-white font-semibold text-sm`}
//                   >
...
//               <div key={user.id} className="flex items-start justify-between pb-1">
//                 <div className="flex gap-3">
//                   <div className="w-10 h-10 rounded-full overflow-hidden">
//                     <img
//                       src={user.avatar}
...
//             <div
//               key={product.id}
//               className="bg-white rounded-lg overflow-hidden border border-gray-200 shadow group hover:shadow-md transition"
//             >
//               <img
//                 src={product.image}
//                 alt={product.name}
...
export default function Dashboard() {
  ...
  const [registrationData, setRegistrationData] = useState([]);
  const [ordersData, setOrdersData] = useState([]);
  const [productSales, setProductSales] = useState({ year: [], month: [], week: [] });
  const [productSalesData, setProductSalesData] = useState([]);
  const [sellers, setSellers] = useState([]);
  const [customers, setCustomers] = useState([]);
  const [popularProducts, setPopularProducts] = useState([]);
  ...
  const fetchDashboardData = async (showLoading = true) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setRegistrationData(stats.registrations || []);
        setOrdersData(stats.orders || []);
        // Set all product sales filters
        setProductSales(stats.productSales || { year: [], month: [], week: [] });
        setSellers(sellers || []);
        setCustomers(customers || []);
        setPopularProducts(products || []);
      } else {
        if (showLoading) {
          ...
          console.error(result.error || "Failed to fetch dashboard data");
        }
      }
    } catch (err) {
      if (showLoading) {
  ...
  // Real-time polling every 10 seconds (adjust interval as needed)
  useEffect(() => {
    const interval = setInterval(() => {
      fetchDashboardData(false); // Background fetch without loading/error UI changes
    }, 5000); // 10 seconds
    return () => clearInterval(interval);
  }, []);

  // Update product sales data based on current filter and productSales
  useEffect(() => {
    const sales = productSales[filter] || [];
    setProductSalesData(
